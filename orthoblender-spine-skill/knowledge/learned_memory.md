...
## LM-0048 — 2026-09-08 — Any smoothing scale expressed in element counts silently halves when the mesh is subdivided
The rim mollifier's passes and gate are counted in rim edges. Subdivide Scan doubled the rim's vertices per mm, so the same passes smooth half the wavelength and the outline's sawtooth reappears at narrow feathers (DEC-0063). Same law as ERR-0034 (density-relative refinement floor): a quality scale must be in millimetres. Also: a feather narrower than ~1.5x the amount gives a wall steeper than 60 deg no matter how dense the mesh — density fixes sampling, never the authored slope (LM-0044) — so "make it smoother" first means "wider feather", and the UI should say the resulting slope out loud.

## LM-0049 — 2026-09-08 — Measure the corner, not the wall: "spikes inside the pad" were the profile's top fillet at 1 mm radius
Splitting "plateau" into core (w>=0.999) and shoulder (0.95<w<0.999) moved 56 of 58 spikes into 82 shoulder vertices; the flat core was clean. The same data read as "plateau roughness" would have blamed the base surface or the direction field (both tested, both innocent). Rule: when a metric band mixes a flat region with a corner, split it before concluding; and compute the analytic corner radius of the authored profile (r = f^2/(6A) for smoothstep) — if it is at the edge-length scale the mesh cannot draw it and no amount of repair will.

## LM-0050 — 2026-09-08 — A scripted RNA property edit pushes no undo step; push one before asserting on undo
Blender's UI pushes an undo step after every slider release, but `region.feather_mm = 25.0` from a timer script does not. `undo_push(); edit; undo()` therefore jumped to the step BEFORE the push (the imported object vanished: "key 'Brace Sample' not found"). Pattern for tests: `undo_push("checkpoint"); edit; undo_push("edit"); undo()`, then re-fetch every ID by NAME — undo rebuilds the structs and every held reference raises "StructRNA ... has been removed".
Also: an `update=` callback on a PropertyGroup inside a CollectionProperty gets the owning Object as `self.id_data`; guard on the identity field (`surface_mask`) being set, because operators assign properties one by one while the region is still half-built.

## LM-0051 — 2026-09-08 — A corner needs 0.2·r edges AFTER displacement; on a 2 mm mesh that is one halving for r ≥ 2.5 mm and nothing for r ≈ 1 mm
The B x4 fixture is 2.03 mm mean edge, not the ~1 mm assumed from the vertex count (DEC-0064 wrote "~1 mm edges" — the count quadruples, the edge only halves). Corner sampling arithmetic: turning per edge = edge / r; at ≤ 0.2 rad the fillet reads as a curve. Measuring the requirement on the STRETCHED post-displacement edge demanded two halvings in the fillet bands (3.25× footprint faces, 30 s); measuring the pre-displacement edge with the floor at half the mesh edge gives one halving (2.4×, 12 s) and already moves the 4/3 mm corners from 21°/43° to 8°/21° (median/max shoulder dihedral). Below r = floor / 0.5 the halving cannot draw the corner and only invites hinge folds (a 1.1 mm smoothstep corner went 54° → 123°): skip it and SAY so (commit note + panel readout: "finer than this mesh draws cleanly (5.1 mm)"). The orthotist's remedies are exactly the product's own buttons: bigger Rounded corners, wider feather, or Subdivide Scan.

## LM-0052 — 2026-09-08 — Three traps from Task 3: fresh BMesh vertices are not index -1 after an op; a relative gate punishes improvement; prove the factor, don't infer it
1. Vertices created by ``bmesh.ops.subdivide_edges`` already carry indices when the op returns (the Python op wrapper numbers its outputs), so "fresh = index < 0" silently matched nothing — a rule that was "on" was off, and a conclusion drawn while it was off ("the face order was the cause") was wrong. Take fresh elements from the op's return dict (``geom_inner``), never from the index.
2. A gate of the form post <= 1.5 x pre fails when pre improves. Pair every relative ceiling with the recorded absolute outcome it was calibrated on.
3. When two changes shipped together (rule + order), the only proof is one factor per run with the OTHER factor genuinely active; the toggle must be verified to do something (a counter, a differing number) before its result is believed.

## LM-0053 — 2026-09-08 — Every bmesh.ops call pays for the whole mesh; and a threshold that is also a repair target defines where the repair STOPS
Rotating a few hundred edges with one bmesh.ops.rotate_edges call each measured 22 s on a 180k-vertex mesh versus well under a second batched — the operator setup (index/normal ensure) is O(mesh) per call. Batch every bmesh.ops call; if one element must be excluded, filter the list, never loop the op.
A guard threshold used as a repair target (hinge 100°) means the surface ends up just under it (measured 98.6°): the guard removes the failure class, it does not beautify. Say so in the record, and keep the threshold above every legitimate authored crease (walls measured ≤ 70° per edge) so it never fights the orthotist's shape.

## LM-0054 — 2026-09-08 — An INWARD feather wider than the pad's half-width erases the pad and exposes the outline's medial axis as a crease
Measured (tools/featherdbg.py; B scan x4, 2 mm edges, circle r 40 mm, 20 mm PRESSURE, Smooth): at Feather 50 the clamp f_eff = min(f, max d) = 38.6 mm leaves 2.3 % of painted vertices above w 0.95, median realized depth 3.1 mm of 20, and only 3 vertices at w >= 0.999. Every crease past 15 deg in the live preview sits in the wall/core (core max 99.3 deg, rim 0) — the distance-to-outline field's medial-axis ridge, which smoothstep hides only where t ~ 1. At Feather 5 the pad keeps its 20 mm but the commit creases the rim (467 edges past 30 deg, max 96.7): the orthotist's two screenshots are the two ends of the same clamp. The orthotist's stated semantics (paint = full-depth contact footprint, feather hangs OUTSIDE onto the body, as Blender proportional edit / Meshmixer) removes both: inside is flat by definition and the outward field has no medial axis within the band. Codex round 1 had explicitly deferred this to the orthotist ("must be established, not inferred"). Cost of an outward band at f50/r40: 5.9k extra vertices (3.6x the pad).

## LM-0055 — 2026-09-09 — When a float32 attribute IS the definition, compute the first evaluation from the float32 values too
Band weights computed from float64 walk distances at Add differed by one ulp (5.96e-8) from every later re-evaluation that reads the float32 attribute back; two "bit-identical" gates failed on storage, not semantics. Rule: round to the stored precision BEFORE the first evaluation (`_stored_band_mm`), and keep tolerances honest about float32 elsewhere (legacy gate: <= 1e-6).

## LM-0056 — 2026-09-09 — Stacked NORMAL-direction displacements are not a vector sum; gate the invariants, not the arithmetic
Two overlapping DISPLACE previews: the second displaces along the normals of the surface the first produced. Where the first wall turns the normals, |combined - (d1 + d2)| measured 21 mm on 17 mm displacements. The product contract is sequential stacking: no attenuation, no normalisation, deterministic — |actual| between the larger single displacement and their sum, identical on re-evaluation. A sum gate would have forced a fixed-direction preview nobody asked for.

## LM-0057 — 2026-09-09 — Refinement-born vertices read 0.0 / False in every OTHER region's point attributes
Blender fills new mesh elements with the attribute default. Under a signed distance encoding 0.0 means "pad vertex on the outline", so a commit that refines one region would silently make its new vertices members of every other live region at full weight. Write the non-member sentinel explicitly after refinement (`_mark_refined_nonmember`); more generally, never let an attribute's default value be a meaningful member state.

## LM-0058 — 2026-09-09 — A guard threshold that is also a repair target stops just under the threshold; the outward field gives the ladder a way OUT (fallback), not a beautifier
The outward EASEOUT arm ships worst 97.3 deg under a 100 deg guard because the refined attempt fails (670 defective faces) and the ladder falls back to the unrefined commit. Without the analytic field the refined commit passed validation WITH 168 deg folds (ERR-0040): the field changed which path ships, it did not fix the validator. Keep tools/hingedbg.py for the next ladder question.
