...
## ERR-0038 — 2026-09-08 — Beauty triangulation of a single-cut split quad stacks four faces on the old edge
Where: _refine_footprint, region_ops.py (the n-gons left by bmesh.ops.subdivide_edges cuts=1 were handed to bmesh.ops.triangulate).
Symptom: the first corner-aware refinement (#54 Task 3) added 5190 vertices and left 6 non-manifold edges, so the commit ladder correctly refused it and fell back to the unrefined commit — silently, from the orthotist's side (refined_added 0, no message about why).
Cause: when the split midpoint is not Phong-lifted (endpoint normals nearly equal, exactly the fillet bands) it lies exactly on the old edge line, so each side's quad has a 180° corner. Blender's beauty rule is undefined there and picked the OLD edge as the diagonal on BOTH sides: two zero-area triangles plus the two real ones on one edge (probe tools/cornerdbg2.py: faces (A,C,M), (A,M,C), (A,C,Q), (A,P,C) on edge A–C).
Fix: _split_refined_ngons — ONLY the quad whose fresh midpoint is unlifted (collinear with the old edge, 1e-4 relative) is split midpoint → opposite vertex; every other n-gon keeps beauty (a fixed 1→2/1→3/1→4 pattern everywhere measurably smoothed worse afterwards, regionqualtest w49f). Fresh vertices come from the subdivide op's return value, not from index -1 (LM-0052). Non-manifold 6 → 0, same commit; the face order handed to triangulate is unchanged.
Prevention rule: never hand a degenerate polygon (a vertex on an edge line) to an angle-optimising triangulator; when you created the midpoint, you know the diagonal — use it.

## ERR-0039 — 2026-09-08 — Inward feather clamp erases the pad when Feather >= the painted area's half-width (orthotist: "the depth disappeared", vertical streaks) — OPEN, fix = Task 7 (DEC-0070)
Symptom: 20 mm PRESSURE, Feather 50, Smooth on an ~80 mm painted area gives median A·w 3.1 mm, 3 vertices at full depth, a medial-axis crease up to 99° in the live preview (LM-0054). The other end: Feather 5 keeps the depth but commits a rim of 467 edges > 30° (max 96.7°). Not a displacement bug, not (shown to be) mesh connectivity; the feather is measured INWARD and clamped to the region's own depth, so a wide feather converts the whole pad into transition. Codex round 3 concurs. Fix: outward band semantics for newly painted regions (Task 7); legacy regions keep the clamp deliberately (removing it under inward semantics would worsen the case). Meanwhile the orthotist's workaround: Feather no larger than about a third of the painted area's width, Rounded corners at or above the reported drawable corner.

## ERR-0040 — 2026-09-09 — Refined commit on the INTERPOLATION path shipped 168.5 deg folds past the hinge guard (outward EASEOUT arm, before the outward analytic field) — OPEN, not reproducible on the analytic path
Observed: tools/hingetest.py EASEOUT arm with an outward region and `_authored_rim_field` returning None: FINISHED, refined_added=1355, 3 footprint pairs past 100 deg (162.5 / 168.5 / 168.1), two of the three involving refinement-born vertices with 4.7–7.0 mm edges on a 2 mm mesh (dissolve products). The ladder's `_folded_pairs` check runs on face-index pairs and `pre_face_normals` computed per retry, so the escape is either a pair outside `affected` at the last check or a fold created after it. With `_outward_rim_field` in place the same arm's refined attempt fails repair (670 remaining), the ladder falls back to the unrefined commit and ships 0 folds (worst 97.3 deg). Trace tool: tools/hingedbg.py (wraps `_folded_pairs`, `_repair_folds`, `_apply_dissolve`, then an independent census). Still reachable by: style/legacy regions whose field fails self-validation. Guard idea for Codex round C: an independent fold census on the FINAL mesh (pairs rebuilt from the shipped topology) as the last gate before FINISHED.

## ERR-0041 — 2026-09-09 — Outward band re-zeroed like the inside put the first band ring at weight 1: the pad grew by a ring (fixed in Task 7)
`_boundary_distance` re-zeroes the level set by the largest rim residual so every painted rim vertex sits at exactly 0; applied outward that shift made band vertices closer to the mollified rim than the worst rim vertex read 0 -> weight 1 -> displaced the full amount (lifecycle gate commit_band_depth: 338 band vertices at 20.000 mm). Fix: the band uses the RAW distance (`evaluate.raw`), strictly positive, stored as -max(d, 1e-6) mm.

## ERR-0042 — 2026-09-09 — Test harness: a helper inserted between `@contextmanager` and `def _phase` stole the decorator; `open_mainfile` under a window `temp_override` is an access violation
The previous session's last edit to tools/featherlifecycletest.py (11:12) placed `_hard_quit` between the decorator and `_phase`, so `with _phase(...)` raised "'generator' object does not support the context manager protocol" and the battery's run hung 34 min. Fixed by moving the decorator back. Earlier the same day: loading a .blend while a `bpy.context.temp_override(window=..., area=...)` is active crashed Blender (EXCEPTION_ACCESS_VIOLATION); the reopen and legacy phases now load OUTSIDE the override and enter it afterwards. After a reload inside a timer, `bpy.context` has no `object` attribute at all — use `view_layer.objects.active` and the override for operator polls.

## ERR-0043 — 2026-09-09 — Outward pads on UNSUBDIVIDED scans (3–4 mm edges) fold the scan's own needle faces inside the pad; refined attempts fall back and the repair slides originals 1–2.5 mm — DISCLOSED (DEC-0072: Commit reports repaired pad folds in its note; a refusal measured as blocking every route; needle-aware directions still open)
Measured (tools/hingedbg.py RIGO_SCAN=sample RIGO_FALLOFF=SMOOTH, Brace Sample 53k verts, regiontest's 300-face patch, 7 mm / feather 10): 48 face pairs folded past 162 deg immediately after displacement, all three vertices of the worst pairs at w = 1.0 inside the pad (faces 3047/31838/10138 around vertices 4723/4726/4727), i.e. a rigid 7 mm move along per-vertex faired normals flips the raw scan's slivers. The inward feather masked this: near the outline those vertices moved a fraction of the amount. Consequences: regiontest max |displacement| 7.069 mm (normal component exact, ~1 mm tangential repair slide), regionstyletest 8.383 (2.5 mm slide), regionqualtest paint15 (A-model) refined_added=0, holes=1, rev=1, smooth_after_commit worsened 5. On the once-subdivided B scan (2 mm edges) the same commits refine and ship clean (hingedbg SMOOTH: 467 refined, repair 0 remaining, worst 89.9 deg). Not fixed in Task 7. Candidates: needle-aware displacement directions (average over the needle's neighbours), or refusing/annotating outward commits whose pad contains needles below the sliver height, or documenting Subdivide Scan as a prerequisite for outward pads on raw scans. Codex round C question.
