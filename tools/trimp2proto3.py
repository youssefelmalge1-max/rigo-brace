"""P2 prototype v3: isolate the C2 solve from the station refinement.

v2 measured two things at once and one of them broke the build, so neither
could be attributed. Findings carried forward from v2:

  - the global periodic C2 solve alone took junction_ratio 9.70 -> 1.01, which
    is the real fix (local tangent rules cannot, v1: 9.91)
  - "subdivide the single worst segment, repeat" produced 5.5-67.1 mm spacing
    (12.2x, WORSE than the 5.4x it started from) because it halves one segment
    at a time, and the resulting build CANCELLED on outer-wall overlap
  - the "corner moved 118 mm" line was a measurement bug: corner identity was
    re-derived by angle AFTER insertion, so inserted stations near the opening
    were mistaken for the originals. Identity is now pinned by position.

This prototype runs four variants from a FRESH curve each time, each followed
by a real Generate, so every number is attributable:

  0  baseline (untouched)
  1  C2 solve only
  2  C2 + balanced refinement (halve EVERY over-tolerance segment per pass,
     preserving all original stations) at 0.6 mm
  3  same at 1.2 mm

Writes trimp2proto3_result.txt; quits Blender itself.
"""
...
OUT = r"C:\Projects\Blender Add-on Braces\trimp2proto3_result.txt"
TRIES = {"n": 0}
CAP = {}
CONTROL_CAP = 120
...
def _projected_spy(base, perimeter):
    ...
    CAP["projected_normals"] = [
        (normal_matrix @ n).normalized() for n in normals
    ]
...
def _split_segments(curve, split_indices):
    """Insert exact De Casteljau midpoints after every listed segment."""
...
def _refine_balanced(context, curve, scan, bvh, tolerance, lines):
    """Halve every over-tolerance segment per pass; originals are preserved."""
...
def _corner_positions(perimeter):
    """World positions of the four opening stations - identity by POSITION."""
...
def _corner_drift(perimeter, recorded):
    """Nearest surviving control to each recorded corner: 0 if preserved."""
    matrix = perimeter.matrix_world
...
def _variant(name, apply_fn, scan, settings, lines):
    ...
    if apply_fn is not None:
        apply_fn(perimeter, scan, bvh, lines)
        perimeter.data.update_tag()
        bpy.context.view_layer.update()
    ratio = _junction_ratio(perimeter.data.splines[0])
    ...
    lines.append(
        f"  {name}: controls={len(perimeter.data.splines[0].bezier_points)} "
        f"spacing {spacing[0]:.1f}-{spacing[1]:.1f}mm ({spacing[2]:.1f}x) "
        f"junction_ratio={ratio:.2f}"
    )
    lines.append(
    ...
    try:
        ...
        result, error = {"CANCELLED"}, str(exc).strip()[:110]
...
def _run():
    ...
    try:
        ...
        outcomes = {}
        outcomes["baseline"] = _variant("0 BASELINE", None, scan, settings, lines)
        outcomes["c2"] = _variant(
            "1 C2 ONLY",
            lambda perimeter, scan, bvh, lines: _apply_c2_handles(
                perimeter.data.splines[0]
            ),
            scan,
            settings,
            lines,
        )
        for tolerance in (0.0006, 0.0012):
            def _apply(perimeter, scan, bvh, lines, tolerance=tolerance):
                _apply_c2_handles(perimeter.data.splines[0])
                _refine_balanced(
                    bpy.context, perimeter, scan, bvh, tolerance, lines
                )

            outcomes[f"c2+{tolerance}"] = _variant(
                f"2 C2 + REFINE {tolerance*1000:.1f}mm",
                _apply,
                scan,
                settings,
                lines,
        ...
        lines.append("")
        for name, (ok, ratio, drift, built) in outcomes.items():
            tangential = f"{built[3]:.3f}" if built else "n/a"
            lines.append(
                f"  {name}: build={'OK' if ok else 'FAILED'} "
                f"ratio={ratio:.2f} corner_drift={drift:.4f} "
                f"tangential_max={tangential}"
            )
